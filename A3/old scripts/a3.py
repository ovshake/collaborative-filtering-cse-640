...
DATA_PATH="FlickscoreData-26oct2018/"
...
class Model:
    def __init__(self, args, rating_matrix, users, movies, movie_ids, ratings):
        ...
        self.negNum = 7
        self.testNeg = self.getTestNeg(self.ratings, 99)
    ...
    def run_epoch(self, sess, verbose=10):
        train_u, train_i, train_r = self.getInstances(self.ratings, self.negNum)
...
def retrieve_ratings_matrix(movies, users, movie_ids):
	ratings_list=[]
	...
	for i in range(len(ratings_of_users)):
		for key in list(ratings_of_users[i].keys()):
			# print(ratings_of_users[i])
			if('submit' not in key):
				user_item_matrix[i][movie_ids[key]]=float(ratings_of_users[i][key][0])
				li=[]
				li.append(float(i))
				li.append(float(movie_ids[key]))
...
RATINGS_MATRIX, RATINGS=retrieve_ratings_matrix(MOVIES, USERS, MOVIE_IDS)
...
classifier = Model(args, RATINGS_MATRIX, USERS, MOVIES, MOVIE_IDS, RATINGS)
